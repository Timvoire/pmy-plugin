...
@listener(command="ss", description="涩涩 末尾添加 `s` 将启用防剧透功能",
          parameters="[自定义频道] [s]")
async def lsp(client: Client, message: Message):
    try:
        ...
        channels = [
            "EnjoyDances", # 辣妹叮叮当当  
            "SportsBeauties", # 健身小姐姐 
            "killerbody", # 🐻🍑 大奶蜜桃臀 
            "EnjoyJK", # JK yyds 制服诱惑 
            "pantyhoselover", # 黑丝 白丝  最爱 
            "TopAcg", # 轻度二次元  
            "sssnnnc", # 少女自拍
        ]
        spoiler = False
        ...
        m = await message.edit(f'[获取涩涩...](https://t.me/{channel})', disable_web_page_preview=True)
        # print(m)
        ...
        await message.edit(f'[获取涩涩 {random_offset}/{count}...](https://t.me/{channel})', disable_web_page_preview=True)
        v = None
        async for m in client.search_messages(chat_id=channel, offset=random_offset, limit=1, filter=enums.MessagesFilter.VIDEO):
            v = m
            await message.edit(f'[获取涩涩视频...](https://t.me/{channel}/{m.id})', disable_web_page_preview=True)
            if spoiler:
                video = await client.download_media(m.video.file_id,in_memory=True)
            else:
